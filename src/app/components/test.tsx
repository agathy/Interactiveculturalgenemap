...
const L1_NODES = [
  { id: 'genzhu', name: '根祖文化', colorKey: '根祖文化', centerText: '根祖' },
  { id: 'zhongyi', name: '忠义文化', colorKey: '忠义文化', centerText: '忠义' },
  { id: 'shanhe', name: '山河文化', colorKey: '山河文化', centerText: '山河' },
  { id: 'gujian', name: '古建文化', colorKey: '古建文化', centerText: '古建' },
  { id: 'jiuhun', name: '酒魂文化', colorKey: '酒魂文化', centerText: '酒魂' },
];
...
export function BreathingNodes({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      L1_NODES.forEach(({ id, name, colorKey, centerText }) => {
        ...
        // 7. 一级节点中心文字
        ctx.save();
        const fontSize = Math.round(l1NodeSize * 0.21);
        ctx.font = `bold ${fontSize}px "KingHwa_OldSong", "Source Han Sans", serif`;
        ...
        ctx.shadowBlur = 0;
        ctx.globalAlpha = 1;
        ctx.strokeText(centerText, x, y);
        // 主色填充 + 发光投影
        ctx.shadowBlur = 14;
        ctx.shadowColor = nodeColor;
        ctx.fillStyle = '#ffffff';
        ctx.globalAlpha = 0.92 + Math.sin(breathRef.current * 0.8) * 0.05;
